...
def copy_details():
    ...
    try:
        ...
        if copy_skills:
            ...
            for skill in skills:
                print(f"Adding skill {skill.id} with proficiency {skill.proficiency} to user {to_user_id}")
                add_user_skill(users_api_instance, to_user_id, skill.id, skill.proficiency)
