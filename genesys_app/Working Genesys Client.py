...
def get_users(users_api_instance):
    try:
        users = users_api_instance.get_users()
        print(f"Fetched {len(users.entities)} users")
...
def add_user_to_queue(routing_api_instance, user_id, queue_id):
    try:
        ...
        body.id = user_id
        routing_api_instance.post_routing_queue_members(queue_id, [body])
        print(f"Added user {user_id} to queue {queue_id}")
    except ApiException as e:
        print(f"Exception when calling post_routing_queue_members: {e}\n")

def add_user_skill(users_api_instance, user_id, skill_id):
    try:
        skill = PureCloudPlatformClientV2.UserRoutingSkillPost()
        skill.routing_skill_id = skill_id
        skill.proficiency = 1.0  # Set a default proficiency, this might need to be adjusted based on your needs
...
def create_phone(telephony_providers_edge_api_instance, user_id, phone_name, site_id):
    try:
        phone_base_id = "57044d99-bcd1-4419-aef0-e66a4021dea2"
        line_base_id = "fad2be41-7894-4e7e-a5ee-ccc1d10fa8bc"
        phone_body = {
            "name": phone_name,
            "site": {"id": site_id},
